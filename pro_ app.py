...
# تحويل النص إلى صوت منطوق بخيار مزدوج
def speak_text(text, filename="response.mp3"):
    if gtts_available:
        try:
            ...
            st.warning(f"⚠️ خطأ في gTTS: {e}")
    elif pyttsx3_available:
        try:
            engine = pyttsx3.init()
            engine.say(text)
            engine.runAndWait()
            st.info("🔊 تم تشغيل الصوت عبر pyttsx3 (محلي).")
        except Exception as e:
            st.warning(f"⚠️ خطأ في pyttsx3: {e}")
    else:
        st.info("📝 الرد الكتابي فقط (لا توجد مكتبات صوتية مثبتة).")
