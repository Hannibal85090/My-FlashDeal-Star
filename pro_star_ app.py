...
# التحكم في المنزل والسيارة (مع معرفات مختلفة لتجنب التكرار)
...
if st.button("إتمام الصفقة"):
    ...
    if deal_text:
        st.session_state.memory_log.append(f"صفقة جديدة: {deal_text} - {now.strftime('%Y-%m-%d %H:%M:%S')}")
